...
void getPosition(const SMikkTSpaceContext *pContext, float position[3], const int face, const int vert)
{
    MeshData *mesh = static_cast<MeshData *>(pContext->m_pUserData);
...
void getNormal(const SMikkTSpaceContext *pContext, float normal[3], const int face, const int vert)
{
    MeshData *mesh = static_cast<MeshData *>(pContext->m_pUserData);
    int index = (*mesh->m_indices)[mesh->m_firstIndex + face * 3 + vert];
    const Model::Vertex &vertex = (*mesh->m_vertices)[index];
...
void getTexCoord(const SMikkTSpaceContext *pContext, float texCoord[2], const int face, const int vert)
{
    MeshData *mesh = static_cast<MeshData *>(pContext->m_pUserData);
    int index = (*mesh->m_indices)[mesh->m_firstIndex + face * 3 + vert];
    const Model::Vertex &vertex = (*mesh->m_vertices)[index];
...
void setTSpaceBasic(const SMikkTSpaceContext *pContext, const float tangent[3], const float sign, const int face,
                    const int vert)
{
