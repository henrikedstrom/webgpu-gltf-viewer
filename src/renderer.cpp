...
void InitGraphics(uint32_t width, uint32_t height)
{
...
void Renderer::Initialize(GLFWwindow *window, Camera *camera, uint32_t width, uint32_t height,
                          const std::function<void()> &callback)
{
    ...
    GetAdapter([window, width, height, callback](wgpu::Adapter a) {
        adapter = a;
        GetDevice([window, width, height, callback](wgpu::Device d) {
            device = d;
